...
@Component
@Handler(supports= FPEmergencyContraceptiveQtyDispDefinition.class, order=50)
public class FPEmergencyContraceptiveQtyDispEvaluator implements EncounterDataEvaluator {
    @Autowired
    ...
    public EvaluatedEncounterData evaluate(EncounterDataDefinition encounterDataDefinition, EvaluationContext context) throws EvaluationException {
        ...
        String qry = "SELECT \n" +
                "    f.encounter_id,\n" +
                "    CASE \n" +
                "        WHEN f.contraceptive_dispensed = 160570 \n" +
                "        THEN CAST(f.quantity_dispensed AS UNSIGNED)\n" +
                "        ELSE 0\n" +
                "    END AS emergency_contraceptives_qty\n" +
