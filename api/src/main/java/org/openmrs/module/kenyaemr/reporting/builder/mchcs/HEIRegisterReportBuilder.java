...
public class HEIRegisterReportBuilder extends AbstractHybridReportBuilder {
	...
	@Autowired
	private PMTCTIndicatorLibrary heiIndicators;
    ...
    protected List<Mapped<DataSetDefinition>> buildDataSets(ReportDescriptor descriptor, ReportDefinition report) {
        ...
        return Arrays.asList(
				...
				ReportUtils.map(heiRegisterAggregateDataSet(), "startDate=${startDate},endDate=${endDate}")
        );
    }
	...
	protected DataSetDefinition heiRegisterAggregateDataSet() {
		CohortIndicatorDataSetDefinition cohortDsd = new CohortIndicatorDataSetDefinition();
		cohortDsd.setName("heiIndicators");
		cohortDsd.addParameter(new Parameter("startDate", "Start Date", Date.class));
		cohortDsd.addParameter(new Parameter("endDate", "End Date", Date.class));

		String indParams = "startDate=${startDate},endDate=${endDate}";

		cohortDsd.addColumn("DNA PCR < 8 weeks", "", ReportUtils.map(heiIndicators.dnaPCRUnder8Weeks(), indParams), "");
		cohortDsd.addColumn("DNA PCR 8-12 weeks", "", ReportUtils.map(heiIndicators.dnaPCR8To12Weeks(), indParams), "");
		cohortDsd.addColumn("Given CTX/DAP within 2 months", "", ReportUtils.map(heiIndicators.ctxDapWithin2Months(), indParams), "");
		cohortDsd.addColumn("EBF at 6 months", "", ReportUtils.map(heiIndicators.ebfUpto6Months(), indParams), "");
		cohortDsd.addColumn("ERF at 6 months", "", ReportUtils.map(heiIndicators.erfUpto6Months(), indParams), "");
		cohortDsd.addColumn("MF at 6 months", "", ReportUtils.map(heiIndicators.mfAt6Months(), indParams), "");
		cohortDsd.addColumn("Not Breastfeeding at 6 months", "", ReportUtils.map(heiIndicators.notBFAt6Months(), indParams), "");
		cohortDsd.addColumn("BF at 12 months", "", ReportUtils.map(heiIndicators.bfAt12Months(), indParams), "");
		cohortDsd.addColumn("Not Breastfeeding at 12 months", "", ReportUtils.map(heiIndicators.notBFAt12Months(), indParams), "");
		cohortDsd.addColumn("BF at 18 months", "", ReportUtils.map(heiIndicators.bfAt18Months(), indParams), "");
		cohortDsd.addColumn("Not Breastfeeding at 18 months", "", ReportUtils.map(heiIndicators.notBFAt18Months(), indParams), "");
		cohortDsd.addColumn("Antibody test at 18 months", "", ReportUtils.map(heiIndicators.antibodyTestAt18Months(), indParams), "");
		cohortDsd.addColumn("Antibody test after 18 months", "", ReportUtils.map(heiIndicators.antibodyTestAfter18Months(), indParams), "");
